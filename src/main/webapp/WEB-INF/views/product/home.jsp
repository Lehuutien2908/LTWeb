...
<head>
    ...
    <style>
        .product-img { height: 180px; object-fit: contain; padding: 10px; }
        .product-card { transition: 0.3s; height: 100%; border-radius: 8px; }

        /* Cấu trúc 2 hàng cuộn ngang - BẮT BUỘC PHẢI CÓ grid-auto-flow: column */
        .slider-viewport { overflow: hidden; width: 100%; position: relative; cursor: grab; user-select: none; }
        .slider-wrapper {
            display: grid;
            grid-template-rows: repeat(2, auto); /* Tạo 2 hàng */
            grid-auto-flow: column;              /* Ép sản phẩm xếp theo cột từ trái sang phải */
            grid-auto-columns: 25%;              /* Hiển thị 4 cột (tổng 8 máy) */
            gap: 15px 0;
...
<body>
                        ...
                        <c:forEach var="p" items="${newProducts}">
                            <div class="slider-item">
                                <div class="product-card text-center d-flex flex-column p-2 border bg-white shadow-sm">
                                    <a href="detail?id=${p.id}"><img src="${pageContext.request.contextPath}/assets/images/${p.image}" class="product-img w-100"></a>
                                    <div class="flex-grow-1 mt-2">
                                        <h6 class="text-dark text-truncate px-2 fw-bold">${p.name}</h6>
                                        <div class="product-price mb-2"><fmt:formatNumber value="${p.price}" pattern="#,###"/>₫</div>
                                    </div>
                                    <button class="btn btn-buy w-100 py-1">MUA HÀNG</button>
                                </div>
                            </div>
                        </c:forEach>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        function setupSliderLogic(wrapperId) {
            ...
            let currentPos = 0;
            const columnsVisible = 4; // Hiển thị 4 cột (8 máy)
            const totalItems = wrapper.children.length; // 12 sản phẩm
            const totalCols = Math.ceil(totalItems / 2); // 6 cột thực tế
            const maxPos = totalCols - columnsVisible; // Cuộn tối đa 2 cột (hiện 4 máy ẩn)

            function update() {
                // Trượt theo % của độ rộng cột (25% mỗi cột trên PC)
                wrapper.style.transform = `translateX(-${currentPos * 25}%)`;
            }

            nextBtns.forEach(btn => btn.onclick = () => {
                currentPos = (currentPos < maxPos) ? currentPos + 1 : 0; // Click trượt 1 cột = 2 máy dọc
                update();
            });
            ...
            viewport.onmousedown = (e) => { isDown = true; startX = e.pageX; scrollPos = currentPos; wrapper.style.transition = 'none'; };
            window.onmouseup = () => { isDown = false; wrapper.style.transition = 'transform 0.5s ease-out'; };
